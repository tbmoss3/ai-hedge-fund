...
class CharlieMungerMemoOutput(BaseModel):
    ...
    target_price: float = Field(description="Target price based on FCF multiple valuation")
...
def generate_charlie_munger_memo(
        ticker: str,
        analysis_data: dict[str, any],
        current_price: float,
        state: AgentState,
        agent_id: str = "charlie_munger_agent",
) -> CharlieMungerMemoOutput:
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are Charlie Munger generating a detailed investment memo.
...
6. A target price based on moat, management quality, and mental models with FCF-based valuation
...
Return JSON only with exactly these fields:
{
  ...
  "target_price": float
    ...
    def create_default_memo():
        return CharlieMungerMemoOutput(
            signal="neutral",
            confidence=50,
    ...
    return call_llm(
        prompt=prompt,
        pydantic_model=CharlieMungerMemoOutput,
        agent_name=agent_id,
        state=state,
...
def run_charlie_munger_with_memo(
    state: AgentState,
    agent_id: str = "charlie_munger_agent"
) -> tuple[dict, dict[str, Optional[InvestmentMemo]]]:
    """
    Run Charlie Munger analysis and generate InvestmentMemo if conviction >= 70%.
    ...
    result = charlie_munger_agent(state, agent_id)
    ...
    for ticker in tickers:
        ...
        if should_generate_memo(confidence) and signal != "neutral":
            ...
            # Generate memo
            memo_output = generate_charlie_munger_memo(
                ticker=ticker,
