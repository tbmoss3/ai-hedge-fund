...
class PhilFisherMemoOutput(BaseModel):
...
def generate_phil_fisher_memo(
        ticker: str,
        analysis_data: dict[str, any],
        current_price: float,
        state: AgentState,
        agent_id: str = "phil_fisher_agent",
) -> PhilFisherMemoOutput:
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are Phil Fisher generating a detailed investment memo.
...
6. A target price based on scuttlebutt, growth potential, R&D investment, and management quality
...
Return JSON only with exactly these fields:
{
  ...
  "target_price": float
    ...
    return call_llm(
        prompt=prompt,
        pydantic_model=PhilFisherMemoOutput,
        agent_name=agent_id,
        state=state,
...
def run_phil_fisher_with_memo(
    state: AgentState,
    agent_id: str = "phil_fisher_agent"
) -> tuple[dict, dict[str, Optional[InvestmentMemo]]]:
    """
    Run Phil Fisher analysis and generate InvestmentMemo if conviction >= 70%.
    ...
    result = phil_fisher_agent(state, agent_id)
    ...
    for ticker in tickers:
        ...
        if should_generate_memo(confidence) and signal != "neutral":
            ...
            # Generate memo
            memo_output = generate_phil_fisher_memo(
                ticker=ticker,
                analysis_data=analysis_data,
                current_price=current_price,
                state=state,
