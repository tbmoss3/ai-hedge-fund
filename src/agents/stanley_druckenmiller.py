...
class StanleyDruckenmillerMemoOutput(BaseModel):
...
def generate_stanley_druckenmiller_memo(
        ticker: str,
        analysis_data: dict[str, any],
        current_price: float,
        state: AgentState,
        agent_id: str = "stanley_druckenmiller_agent",
) -> StanleyDruckenmillerMemoOutput:
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are Stanley Druckenmiller generating a detailed investment memo.
...
6. A target price based on macro regime, momentum, and risk/reward asymmetry
...
Return JSON only with exactly these fields:
{
  ...
  "target_price": float
    ...
    def create_default_memo():
        return StanleyDruckenmillerMemoOutput(
            signal="neutral",
            confidence=50,
    ...
    return call_llm(
        prompt=prompt,
        pydantic_model=StanleyDruckenmillerMemoOutput,
        agent_name=agent_id,
        state=state,
...
def run_stanley_druckenmiller_with_memo(
    state: AgentState,
    agent_id: str = "stanley_druckenmiller_agent"
) -> tuple[dict, dict[str, Optional[InvestmentMemo]]]:
    """
    Run Stanley Druckenmiller analysis and generate InvestmentMemo if conviction >= 70%.
    ...
    result = stanley_druckenmiller_agent(state, agent_id)
    ...
    for ticker in tickers:
        ...
        if should_generate_memo(confidence) and signal != "neutral":
            ...
            # Generate memo
            memo_output = generate_stanley_druckenmiller_memo(
                ticker=ticker,
                analysis_data=analysis_data,
                current_price=current_price,
                state=state,
